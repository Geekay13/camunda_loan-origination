...
const createLoan = async (requestData) => {
  const { first_name, last_name, address, contact, pi, loan } = requestData;
    console.log("Received Data:", requestData);
    const processVariable = requestData;
  // Check if user exists by SSN
  let existingUser = await LoanApplication.findOne({ "pi.ssn": pi.ssn });
  ...
  await newLoan.save();
  return await startProcess(processVariable);  
};

module.exports = { handleLoanApplication, createLoan }
